...
function needsApproval(name, args, { mode, cwd }) {
 ...
 switch (name) {
  ...
  case 'run_bash': return ask || riskyShell(args.command, cwd);
...
function describe(name, args, cwd) {
 ...
 switch (name) {
  case 'run_bash': return { kind: 'command', title: I18n.t('approve.command'), code: String(args.command || '') };
...
function format(name, args, result) {
 ...
 switch (name) {
  ...
  case 'git':
   if (result.missing) return 'Error: Git is not installed. Install it with the system package manager, for example: sudo apt install git';
