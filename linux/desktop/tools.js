...
function detectShell() {
 shell ||= run('', BASH.exe, ['-c', 'printf %s "$BASH_VERSION"'], { cwd: os.homedir(), timeout: TIMEOUT.probe })
  .then(result => result.code === 0 && result.output ? { ...BASH, version: result.output.trim() } : BASH);
 return shell;
}
...
async function runShell(id, { command, timeout }, cwd) {
 ...
 await fs.promises.writeFile(file, `#!/bin/bash\nexport LANG=C.UTF-8\n${command}\n`, { mode: 0o700 });
...
async function readFile(id, { path: file, offset, limit }, cwd) {
 ...
 if (stat.size > READ.max) return { error: `${file} is ${formatSize(stat.size)}, too big to read at once. Read parts of it with run_bash (sed -n, grep).` };
