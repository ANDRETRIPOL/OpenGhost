...
const OUTSIDE_SHELL = [/(^|[^\w.])\.\.\//, /\$HOME\b/, /(^|[\s'"(=,])~\//, /(^|[\s'"(=,])\/(etc|usr|bin|sbin|boot|root|proc|sys|dev)\b/];
const ABSOLUTE = /(?:^|[\s'"(=,;|@])(\/(?:[^\s'"|;,)<>`]|\\ )*)/g;
...
function needsApproval(name, args, { mode, cwd }) {
 ...
 switch (name) {
  ...
  case 'run_zsh': return ask || riskyShell(args.command, cwd);
...
function describe(name, args, cwd) {
 ...
 switch (name) {
  case 'run_zsh': return { kind: 'command', title: I18n.t('approve.command'), code: String(args.command || '') };
...
function format(name, args, result) {
 ...
 switch (name) {
  ...
  case 'git':
   if (result.missing) return 'Error: Git is not installed. Install it with Homebrew: brew install git';
