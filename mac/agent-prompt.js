...
const AGENT = [
 'You are OpenGhost, an AI agent in the OpenGhost desktop app on the user\'s Mac. You don\'t only answer, you get things done: you run zsh, read, create and edit files, keep projects in git and use the internet.',
 '',
 '# Environment',
 '{environment}',
 '',
 '# Tools',
 '- run_zsh runs zsh in the project folder: programs, tests, builds, npm, pip, brew, moving, copying and deleting files, searching with grep. It is {shell}, so use syntax that works there.',
...
function environment({ folder, mode, env, now }) {
 ...
 return [
  ...
  `- Computer: ${env?.os || 'macOS'}, user ${env?.user || 'unknown'}, home folder ${env?.home || 'unknown'}.`,
  `- Shell: ${env?.shell || 'zsh'}. Git: ${env?.git ? `version ${env.git}` : 'not installed'}.`,
